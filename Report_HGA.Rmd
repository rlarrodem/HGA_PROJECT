...
libs_01 <- c("tidyverse", "httr", "rvest", "valr", "forcats", "scales", "ggplot2", "Hmisc", "nortest", "modeest", "remotes")

# Checking installed standard libs
missing_libs <- libs_01[!sapply(libs_01, requireNamespace, quietly = TRUE)]

# Installing of missing libs
if (length(missing_libs) > 0) {
  install.packages(missing_libs)
  text_01 <- "Libraries just installed"
} else {
  text_01 <- "Libraries previously installed"
}
print(text_01)
...
```{r message = FALSE, warning = FALSE}
# Checking and installation of required specialized libraries - BiocManager
...
```{r message = FALSE, warning = FALSE}
# Checking and installation of required specialized libraries - rtracklayer
...
libraries <- c("tidyverse", "httr", "rvest", "valr", "forcats", "scales", "ggplot2", "Hmisc", "nortest", "modeest", "remotes", "rtracklayer")
inactive_libraries <- libraries[!sapply(libraries, function(x) is.element(x, search()))]
if (length(inactive_libraries) > 0) {
  lapply(inactive_libraries, require, character.only = TRUE)
  message <- "Libraries just activated"
} else {
  message <- "Libraries previously activated"
}
print(message)
...
if (!file.exists(fPath)) {
  ...
  print("Data file already available")
}
```
...
# Import data
if ("data" %in% ls()) {
  message("Data previously imported and available in the project.")
} else {
  data <- import(fPath, format = "gff")

  message("Data already imported and available in the project.")
}
...
```{r}
if ("gencode" %in% ls()) {
  message("Tibble 'gencode' already exists in the project.")
} else {
  gencode <- as_tibble(data)
  message("Tibble 'gencode' created.")
}

...
```{r}
gencode <- import('gencode.v43.annotation.gff3')
```

```{r}

gencode2 <- gr_to_bed(gencode)
# dim(gencode2)

```

Note: Not clear why gencode2 has only 8 columns, hence to include all features I did like this:

```{r}
gencode3 <- as_tibble(gencode@elementMetadata)

```

```{r}
gencode3b <- as_tibble(gencode@ranges@width, column_name = "width")
```

```{r}
gencode4 <- bind_cols(gencode2, gencode3b,  gencode3)

```

Finally, `gencode4` is the tibble used for the analysis

Now, it is going to be extracted a random sample from 'gencode4' to simplify code creation:

```{r}
gencode_s <- gencode4[sample(nrow(gencode4), size = 5000),]
...
```{r}
# print("4.1. Dimensions of table gencode4 are:")
dim(gencode4)
...
```{r}
genes_00 <- filter(gencode4, type == "gene")
```
...
```{r}
# url1 = 'http://hgdownload-test.cse.ucsc.edu/goldenPath/hg38/bigZips/hg38.chrom.sizes'
# read_html(url1)

# h1 <- GET(url1=url1, verbose(), config(cainfo="cacert.pem"))
...
```{r}
path_00 = "C:/Users/milagros/Documents/GEN_PROJECT_230501/H_Gen_RL/hg38.txt"
```

```{r}
chSize_00 <- as_tibble(read.table(path_00, header = FALSE, sep = "\t"))

```

```{r}
chr_01 <- fct_count(genes_00$chrom, sort = FALSE, prop = FALSE)
```
...
6.0.2. Merging Datas
```{r}
chSize_01 <- left_join(chrGMeta, chSize_00, by = c("chrom" = "V1"))
...
```{r}
chSize_02 <- filter(chSize_01, meta_categories %in% c("RNA", "prot_coding"))
...
plot_Facet_02 <- ggplot(data = chSize_02) +
  ...
  geom_vline(aes(xintercept=median(chSize_02$dGen_norm)), 
      ...
      linetype = "dashed", 
      linewidth = .2) +
  scale_x_continuous(breaks=seq(0, max(chSize_02$dGen_norm)+100, by = 100), labels = comma_format()) +
  theme_classic() +
  labs(title = "Gene Density per Chromosome",
    subtitle = "Breakdown per Meta_Categories",
    x = "Count of Genes per Mega-Bases",
...
Global Data:
```{r}
chr_Highest <- slice_max(chr_02, dGen_norm)
chr_Highest
```
...
```{r}
geneSize_00 <- select(genes_00, gene_id, gene_name, meta_categories, value)

```

```{r}
geneSize_00 <- mutate(geneSize_00, logSize = log10(value))
...
```{r}
plot_Filled_03 <- ggplot(data = geneSize_S) +
  aes(x = value, fill = meta_categories) +
  geom_density(alpha = .2) +
...
mode_x <- mlv(filter(geneSize_00, meta_categories %in% c(mc))$value)
mean_x <- mean(filter(geneSize_00, meta_categories %in% c(mc))$value)

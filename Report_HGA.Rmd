...
```{r}
gencode_00 <- import('gencode.v43.annotation.gff3')
```

```{r}

gencode2 <- gr_to_bed(gencode_00)
# dim(gencode2)
...
```{r}
gencode3 <- as_tibble(gencode_00@elementMetadata)

```

```{r}
gencode3b <- as_tibble(gencode_00@ranges@width, column_name = "width")
```

```{r}
gencode <- bind_cols(gencode2, gencode3b,  gencode3)
...
```{r}
# print("4.1. Dimensions of table gencode are:")
dim(gencode)
